...
for (const [p, pathItem] of Object.entries(paths)) {
  for (const m of ['get', 'post', 'put', 'delete', 'patch', 'head', 'options', 'trace']) {
    ...
    const directParams = (op.parameters || []).map((x) => resolveRef(resolver, x));
    // path-level params（v2/v3）—可选：这里简单合并
    const pathLevelParams = (pathItem.parameters || []).map((x) => resolveRef(resolver, x));
    ...
    for (const [code, resp] of Object.entries(responses)) {
      ...
      respRows.push({
        ...
        Description: r && r.description ? r.description.replace(/\s+/g, ' ').trim() : '',
      });
    }
  }
}
...
const models = isV3 ? (spec.components && spec.components.schemas) || {} : spec.definitions || {};
for (const [name, schema] of Object.entries(models)) {
  ...
  const propsPreview = s.properties ? Object.keys(s.properties).slice(0, 12).join(',') : '';
  modelRows.push({
    Name: name,
    Type: type,
    Required: required,
...
function previewSchema(s) {
  ...
  if (s.type === 'object' && s.properties) {
    ...
    return `{ ${keys.map((k) => `${k}:${guessSchemaType(s.properties[k])}`).join(', ')}${
...
function expandObjectFirstLevel(rows, base, schema) {
  ...
  for (const [field, fschema0] of Object.entries(props)) {
    const fschema = fschema0 && (fschema0.$ref ? resolver.resolveRef(fschema0) : fschema0);
    rows.push({
      Tag: base.Tag,
      Method: base.Method,
      Path: base.Path,
      ...
      FieldDescription: fschema && fschema.description ? String(fschema.description).replace(/\s+/g, ' ').trim() : '',
    });
  }
}
