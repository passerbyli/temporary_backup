      ...
      (config) => {
        ...
        this._log(
          ...
          [
            ...
            `Params: ${this._formatJSON(config.params || {})}`,
            `Data: ${this._formatJSON(config.data || {})}`,
          ],
          config.meta.tag,
        );
    ...
    this.axios.interceptors.response.use(
      (res) => {
        ...
        this._log(
          '← Response',
          [
            `URL: ${res.config.method.toUpperCase()} ${res.config.url}`,
            `Status: ${res.status}`,
            `Duration: ${duration}ms`,
            `Response: ${this._formatJSON(res.data)}`,
          ],
          meta?.tag,
        );
      ...
      async (error) => {
        ...
        this._log(
          '× Response Error',
          [`URL: ${config.method?.toUpperCase()} ${config.url}`, `Duration: ${duration}ms`, `Error: ${error.message}`],
          tag,
        );
        ...
        if (
          ...
          meta.retry < this.options.retryTimes
        ) {
          meta.retry++;
