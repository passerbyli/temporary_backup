...
function mockUpdateFlow() {
  ...
  setTimeout(async () => {
    ...
    // 3) progress
    let percent = 0;
    if (mainWindow && !mainWindow.isDestroyed()) mainWindow.setProgressBar(0);

    const timer = setInterval(() => {
      percent += 2.5; // 调整速度
      const transferred = (Math.min(100, percent) / 100) * 120 * 1024 * 1024;
      ...
      showUpdateUI({
        ...
        detail: `${formatMB(transferred)} / ${formatMB(total)}  ·  12.0 MB/s`,
      });
      ...
      if (percent >= 100) {
        ...
        setTimeout(async () => {
          ...
          const rr = await dialog.showMessageBox(mainWindow ?? undefined, {
            type: 'info',
            title: '更新已就绪（DEV 模拟）',
            message: '更新已下载完成',
