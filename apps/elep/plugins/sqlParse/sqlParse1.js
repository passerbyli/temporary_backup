...
function parseSql(sqlContent, options = { includeTemporaryTables: true }) {
  ...
  while ((match = procedureRegex.exec(sqlContent)) !== null) {
    const schemaName = match[3],
      name = match[4];
    if (match[2].toUpperCase() === 'PROCEDURE') procedures.push({ type: 'PROCEDURE', schema: schemaName, name });
  ...
  while ((match2 = selectMapRegex.exec(sqlContent)) !== null) {
    ...
    const [targetSchema = 'public', targetTable] = targetTableFull?.split('.') ?? ['public', targetTableFull];
    const [sourceSchema = 'public', sourceTable] = sourceTableFull?.split('.') ?? ['public', sourceTableFull];
