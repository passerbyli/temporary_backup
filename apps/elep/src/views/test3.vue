  ...
  <div style="height: 100vh; display: flex; flex-direction: column">
    <div style="padding: 10px; background: #f0f0f0; display: flex; align-items: center; gap: 20px">
      <input
        ...
        placeholder="搜索表名或过程，如 orders 或 sp_xxx"
        style="width: 300px; padding: 6px"
      />
      <div style="font-size: 12px">
        <span
          v-for="(color, layer) in layerColors"
          :key="layer"
          :style="{
...
const data = {
  ...
  edges: [
    ...
    { source: 'dws.user_profile', target: 'ads.user_dashboard', label: 'DWS → ADS', mapping: ['uid → user_id'] },
    {
      ...
      label: 'CALL input',
      mapping: ['user_id → in_user_id', 'payment_amount → out_amount'],
    },
    {
      source: 'dim.product_info',
      target: 'sp_get_dashboard_data',
      label: 'CALL join',
      mapping: ['product_id → out_user_name'],
    },
  ],
};
