...
async function main(regex, options) {
  ...
  const files = entries
    ...
    .filter((f) => !excludeExts.includes(path.extname(f).toLowerCase()));
...
async function run() {
  ...
  urlRegex =
    /^\s*(?:"?(?:token|key|secret|password)"?\s*[:=]\s*(?:(?<q1>['"])[A-Za-z0-9+/=_-]{8,}\k<q1>|[A-Za-z0-9+/=_-]{8,})\s*[;,]?\s*$|[\w.-]*(?:token|key|secret|password)\b\s*=\s*(?:(?<q2>['"])[^\s#'"]+\k<q2>|[^\s#'"]+)\s*(?:#.*)?$)/g;
