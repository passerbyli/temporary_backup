<template>
  <div>
    ...
    <!-- 表格 -->
    <el-table :data="filteredData" border style="width: 100%" :row-style="getRowStyle">
      <!-- 名称列 -->
      <el-table-column prop="name" label="名称" sortable :filters="getFilters('name')" :filter-method="filterHandler" />
      ...
      <el-table-column
        ...
        label="开发交付时间"
        sortable
        :filters="getFilters('transferTime')"
        :filter-method="filterHandler"
      />

      <!-- testCompletionTime 列 -->
      <el-table-column
        prop="testCompletionTime"
        label="测试完成时间"
        sortable
        :filters="getFilters('testCompletionTime')"
        :filter-method="filterHandler"
      />
    </el-table>
  </div>
