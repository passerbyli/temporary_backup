      ...
      setting: {
        ...
        field: {
          'table.field1_cn': {
            ...
            nodeId: '',
            fieldCfg: {
              list: ['table.字段_中文', 'table.field1_name_cn', 'table.field999_name_cn', 'table.field1_name'],
              isSelectAll: true,
            },
          },
        },
...
function translateTreeId(obj, modelData) {
  if (obj.treeId) {
    ...
    let enObj =
      tableObj.tables.find((t) => t.field === fieldName) || tableObj.measures.find((m) => m.name === fieldName);
      ...
      if (enObj) {
        ...
        if (fieldObj.extra && Array.isArray(fieldObj.extra)) {
          fieldObj.extra.forEach((extraItem) => {
            if (!extraItem.SelectedValues || extraItem.SelectedValues.length === 0) {
              // 处理 extra 中的 field
              const arr = extraItem.field.split('.');
              let extraFieldName = arr[1];
              if (extraFieldName.endsWith('_cn') || extraFieldName.endsWith('_中文')) {
                extraFieldName = extraFieldName.replace('_cn', '_en').replace('_中文', '_英文');
              } else if (extraFieldName.includes('_cn_') || extraFieldName.includes('_中文_')) {
                extraFieldName = extraFieldName.replace('_cn_', '_en_').replace('_中文_', '_英文_');
              }
              extraItem.field = `${arr[0]}.${extraFieldName}`;

              translateSettingField({ field: { [extraItem.field]: extraItem } }, modelData);
            }
          });
        }
        ...
        if (fieldObj.fieldCfg && Array.isArray(fieldObj.fieldCfg.list)) {
          fieldObj.fieldCfg.list = fieldObj.fieldCfg.list.map((listItem) => {
            ...
            if (listFieldName.endsWith('_cn') || listFieldName.endsWith('_中文')) {
              listFieldName = listFieldName.replace('_cn', '_en').replace('_中文', '_英文');
            }
...
try {
  ...
  // 保存新生成的 JSON 文件
  fs.writeFileSync('sheetData_en.json', JSON.stringify(sheetData, null, 2), 'utf-8');
