# 0) 到仓库根目录（按你的路径调整）
cd /Users/lihaomin/projects/GitHub/temporary_backup
...
rm -rf apps/frontend/elep/node_modules/electron apps/frontend/elep/node_modules/.pnpm/electron@*
...
pnpm -w install --filter ./apps/frontend/elep

# 7) 重新执行 Electron 的安装脚本（会下载解压二进制）
pnpm -w rebuild electron --filter ./apps/frontend/elep

# 8) 验证 Electron 二进制是否到位
pnpm -C apps/frontend/elep exec electron --version
ls apps/frontend/elep/node_modules/electron/dist | sed -n '1,50p'

# 9) 启动你的项目（示例，按你的脚本调整）
cd apps/frontend/elep
