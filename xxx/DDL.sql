...
CREATE TABLE  IF NOT EXISTS meta_data_ds.metadata_api (
      ...
      created_by        text                   ,
      updated_by        text                   ,
      CONSTRAINT        chk_metadata_api_method CHECK (,
      api_method        IN                      (,
      'GET'                                    ,
      'POST'                                   ,
      'PUT'                                    ,
...
-- 唯一：请求方式 + 微服务名称 + 接口路径
CREATE UNIQUE INDEX IF NOT EXISTS ux_metadata_api_signature ON meta_data_ds.metadata_api (api_method, microservice_name, api_path);
...
CREATE TABLE  IF NOT EXISTS meta_data_ds.metadata_api_field (
      ...
      created_by      text                        ,
      updated_by      text                        ,
      CONSTRAINT      chk_metadata_api_field_io    CHECK (io_type IN ('in', 'out')),
      CONSTRAINT      chk_metadata_api_field_dtype CHECK (,
      data_type       IS                           NULL,
      OR              data_type                    IN (,
      'string'                                    ,
      'number'                                    ,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_api_field_api_io_path ON meta_data_ds.metadata_api_field (api_id, io_type, path);

-- 常用查询索引：按父子/层级/顺序遍历
CREATE INDEX IF NOT EXISTS ix_api_field_api_parent ON meta_data_ds.metadata_api_field (api_id, io_type, parent_field_id, seq);

CREATE INDEX IF NOT EXISTS ix_api_field_api_level ON meta_data_ds.metadata_api_field (api_id, io_type, level);

CREATE INDEX IF NOT EXISTS gin_api_field_rule_ext ON meta_data_ds.metadata_api_field USING GIN (rule_ext);
...
CREATE TABLE  IF NOT EXISTS meta_data_ds.metadata_api_call (
      ...
      created_by            text              ,
      updated_by            text              ,
      CONSTRAINT            chk_api_call_rtype CHECK (,
      resource_type         IN                 (,
      'table'                                 ,
      'procedure'                             ,
      'api'                                   ,
